...
describe("Post authentication tasks", () => {
  ...
  test("Challenge 4.a - Calculate average user rating for a given movie id", async () => {
    const rating = await feedbackRepository.getAverageUserRatingForMovie(
      "b349436c-a26a-4959-99af-7a1d9c4088fc"
    );
    expect(rating).toBe(3.7);
  });

  test("Challenge 4.b - Get all movies", async () => {
    const res = await authenticatedSession
      .get("/api/movie/")
      .set("Authorization", `Bearer ${authToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data.length).toBe(13);
    expect(res.body.data).toEqual(expectedResponse2);
  });

  test("Challenge 4.c - Get movie by given movie id", async () => {
    const movieId = "e3d0cd0e-4150-4118-a227-329dfae69d9f";
    const res = await authenticatedSession
      .get(`/api/movie/${movieId}`)
      .set("Authorization", `Bearer ${authToken}`);
    expect(res.status).toBe(200);
    expect(res.body.data).toEqual(expectedResponse1);
  });
